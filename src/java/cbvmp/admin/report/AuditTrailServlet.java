...
import cbvmp.admin.report.manager.AuditTrailModelManager;
import cbvmp.admin.report.manager.SimVerConsentUnconsentModelManager;
 ...
 */
@WebServlet(name = "AuditTrailServlet", urlPatterns = {"/report/audit"})
...
//            out.println("<html>");
//            out.println("<head>");
//            out.println("<title>Servlet AuditTrailServlet</title>");            
//            out.println("</head>");
//            out.println("<body>");
//            out.println("<h1>Servlet AuditTrailServlet at " + request.getContextPath() + "</h1>");
//            out.println("</body>");
//            out.println("</html>");
//        }
    }
    ...
    private void generateReport(HttpServletRequest request,
        ...
        try {
            //reportPath = request.getSession().getServletContext().getRealPath("/Jasper") + "\\SimVer.jrxml";

            ClassLoader classLoader = getClass().getClassLoader();
            InputStream input = classLoader.getResourceAsStream("cbvmp/jasper/jrxml/AuditTrail.jrxml");
            reportPath = "AuditTrail.jrxml";
            ...
            AuditTrailModelManager auditManager = new AuditTrailModelManager();
            ...
            list = auditManager.viewReport(cmpoParam, dateFrom, dateTo, groupByTimeParam);
            // for (PolicyMasterModel pm : policyMasterModelManager.listAll()) {
//            for (Iterator<AuditTrailModel> it = list.iterator(); it.hasNext();) {
//                AuditTrailModel pm = it.next();
//                System.out.println(pm.getCmpoName());
//                System.out.println(pm.getGrpByDate());
//            }
//            for  (SimVerConsentUnconsentModel am: list) {
//                System.out.println(am.getCmpoName()+" "+am.getDateGrpBy());
//                
//            }
//            for (int i=0; i<list.size(); i++)
//            {
//                System.out.println(list.get(i).getDateGrpBy());
//            }
            ...
            request.getSession().setAttribute("dFrom", dFrom);
            request.getSession().setAttribute("dTo", dTo);
            request.getSession().setAttribute("groupByTimeParam", groupByTimeParam);
            request.getSession().setAttribute("cmpo", cmpoParam);
            ...
            if (jp.getPages().size() > 0) {
                ...
                response.sendRedirect(request.getContextPath() + "/report/audit");
            } else {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (page != null && (Integer.parseInt(page) <= lastPageIndex)) {
            ...
            request.getRequestDispatcher("/view/report/auditTrail.jsp").forward(request, response);
